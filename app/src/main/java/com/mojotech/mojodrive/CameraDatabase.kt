        ...
        db.rawQuery(
            ...
            while (c.moveToNext()) {
                result += CameraPoint(
                    ...
                    speedLimit = if (c.isNull(idx("speed_limit"))) null else c.getInt(idx("speed_limit")),
                    roadName = if (c.isNull(idx("road_name"))) "" else c.getString(idx("road_name")),
                    roadHighway = if (c.isNull(idx("road_highway"))) "" else c.getString(idx("road_highway")),
                    roadDistanceM = if (c.isNull(idx("road_distance_m"))) null else c.getDouble(idx("road_distance_m")),
                    speedSource = if (c.isNull(idx("speed_source"))) "" else c.getString(idx("speed_source")),
                    speedConfidence = if (c.isNull(idx("speed_confidence"))) "unknown" else c.getString(idx("speed_confidence")),
                    roadBearing = if (c.isNull(idx("road_bearing"))) null else c.getFloat(idx("road_bearing")),
                    oneway = !c.isNull(idx("oneway")) && c.getInt(idx("oneway")) == 1,
                    directionMode = if (c.isNull(idx("direction_mode"))) "unknown" else c.getString(idx("direction_mode")),
                    directionConfidence = if (c.isNull(idx("direction_confidence"))) "unknown" else c.getString(idx("direction_confidence"))
                )
            }
        }
        db.close()
